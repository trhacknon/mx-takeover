...
func expireMXDomain(mx_domain, expire_date string, dmn []string) {
	...
	if days_remain < *expr_day {
		color.Green("[+] %s will be expired after [%d] days. It is being used by %d diffirent domain. Expire Time: [%s]. Domains that are used by this mx:", mx_domain, days_remain, len_dmn, expire_date) // number of days
		fmt.Println(dmn)
	}
}
